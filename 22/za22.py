...
# mc.setBlocks(x-1, y-1, z-1,		x + width+1, y + depth -1, z + length+1, 169)
# mc.setBlocks(x, y-1, z,		x + width, y + depth, z + length, 20)
# # Очищаем внутреннюю часть бассейна
# mc.setBlocks(x+1, y-1, z+1,		 x + width - 1, y + depth + 1, z + length - 1, 0)
# # Заполняем пространство бассейна водой
# mc.setBlocks(x+1, y-1, z+1,		 x + width -1, y + depth + 1, z + length -1, 9)
            ...
            for x in range(len(structure[y][z])):
                ...
                if structure[y][z][x].id != 0:
                    mc.setBlock(x + xStart, y + yStart,
                                z + zStart, structure[y][z][x].id, 
																structure[y][z][x].data)
